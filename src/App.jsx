...
import { chatPath, ebookPath, quizPaths, readRoute, vocabularyLanguagePath, vocabularyPath, vocabularyPracticePath, workspacePath, workspacesPath } from './lib/routes'
...
export default function App() {
  ...
  const [route, setRoute] = useState(initialRoute.page === 'quiz' || initialRoute.page === 'workspaces' || initialRoute.page === 'vocabulary' || initialRoute.page === 'ebook' || initialRouteLanguage || initialRouteChat || initialRouteWorkspace ? initialRoute : { page: 'home', chatId: null })
  ...
  useEffect(() => { const timer = setTimeout(() => setStorageError(!saveState({ theme, providers, activeProvider, models, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages, ebooks, workspaces, conversations, activeId, activeWorkspaceId, temperature, wideChat, sidebarWidth, sidebarVisible, pronunciationLanguage, pronunciationVoice, pronunciationRate })), 300); return () => clearTimeout(timer) }, [theme, providers, activeProvider, models, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages, ebooks, workspaces, conversations, activeId, activeWorkspaceId, temperature, wideChat, sidebarWidth, sidebarVisible, pronunciationLanguage, pronunciationVoice, pronunciationRate])
  useEffect(() => {
    let frame
    let timer
  ...
  useEffect(() => {
    const onPopState = () => {
      ...
      } else if (nextRoute.page === 'workspaces' || nextRoute.page === 'vocabulary' || nextRoute.page === 'ebook') {
        setRoute(nextRoute)
  ...
  useEffect(() => { const vocabularyLanguage = vocabularyLanguages.find((language) => language.id === route.languageId); document.title = route.page === 'chat' ? `${activeConversation?.title || 'Chat'} · ShinkuChat` : route.page === 'workspace' ? `${activeWorkspace?.name || 'Workspace'} · ShinkuChat` : route.page === 'quiz' ? 'Quiz · ShinkuChat' : route.page === 'ebook' ? 'Ebook/PDF Maker · ShinkuChat' : route.page === 'vocabulary-practice' ? `${vocabularyLanguage?.name || 'Vocabulary'} Practice · ShinkuChat` : route.page === 'vocabulary-language' ? `${vocabularyLanguage?.name || 'Vocabulary'} · ShinkuChat` : route.page === 'vocabulary' ? 'My Vocabulary · ShinkuChat' : 'ShinkuChat · Home' }, [route.page, route.languageId, activeConversation?.title, activeWorkspace?.name, vocabularyLanguages])
  ...
  return <div className="relative flex h-dvh overflow-hidden bg-white font-sans text-sm text-slate-800 [&_*]:!font-sans [&_a]:cursor-pointer [&_button:not(:disabled)]:cursor-pointer [&_button:disabled]:cursor-not-allowed [&_code]:!font-mono [&_input:not(:disabled)]:cursor-text [&_input:disabled]:cursor-not-allowed [&_input[type=button]:not(:disabled)]:cursor-pointer [&_input[type=checkbox]:not(:disabled)]:cursor-pointer [&_input[type=color]:not(:disabled)]:cursor-pointer [&_input[type=file]:not(:disabled)]:cursor-pointer [&_input[type=radio]:not(:disabled)]:cursor-pointer [&_input[type=range]:not(:disabled)]:cursor-pointer [&_input[type=reset]:not(:disabled)]:cursor-pointer [&_input[type=submit]:not(:disabled)]:cursor-pointer [&_label]:cursor-pointer [&_option]:cursor-pointer [&_pre]:!font-mono [&_pre_*]:!font-mono [&_select:not(:disabled)]:cursor-pointer [&_select:disabled]:cursor-not-allowed [&_summary]:cursor-pointer [&_textarea:not(:disabled)]:cursor-text [&_textarea:disabled]:cursor-not-allowed [&_[contenteditable=true]]:cursor-text [&_[draggable=true]]:cursor-grab [&_[role=button]]:cursor-pointer [&_[role=checkbox]]:cursor-pointer [&_[role=combobox]]:cursor-pointer [&_[role=link]]:cursor-pointer [&_[role=listbox]]:cursor-default [&_[role=menuitem]]:cursor-pointer [&_[role=option]]:cursor-pointer [&_[role=radio]]:cursor-pointer [&_[role=slider]]:cursor-pointer [&_[role=switch]]:cursor-pointer [&_[role=tab]]:cursor-pointer transition-[background-color,color] duration-500 dark:bg-slate-950 dark:text-slate-100">
    ...
    <aside className={`fixed inset-y-0 left-0 z-30 flex max-w-[calc(100vw-24px)] ${sidebarWidths[sidebarWidth]} flex-col overflow-hidden border-r border-purple-100/70 bg-white/80 p-3 shadow-[16px_0_40px_rgba(76,29,149,0.08)] backdrop-blur-xl transition-transform duration-500 ease-out dark:border-white/5 dark:bg-slate-950/80 dark:shadow-[16px_0_45px_rgba(126,34,206,0.08)] ${sidebarVisible ? 'md:static md:translate-x-0' : 'md:fixed md:-translate-x-full'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="relative space-y-1 border-t border-purple-100 pt-2 dark:border-white/5"><button onClick={() => downloadJson({ workspaces, conversations, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages, ebooks }, 'shinkuchat-export.json')} className={`${button} w-full justify-start gap-2 px-2`}><DownloadSimple size={17} />Export data</button><button onClick={() => setSettingsOpen(true)} className={`${button} w-full justify-start gap-2 px-2`}><GearSix size={17} />Provider & model</button></div>
    </aside>
