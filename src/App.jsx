...
import { ArrowsInLineHorizontal, ArrowsOutLineHorizontal, ArrowDown, ArrowRight, ArrowSquareOut, BookOpen, CaretLeft, CaretRight, ChatCircle, DownloadSimple, Exam, GearSix, House, PaperPlaneRight, Plus, Sparkle, Stop, Trash, X } from '@phosphor-icons/react'
...
import { chatPath, quizPaths, readRoute, vocabularyLanguagePath, vocabularyPath, vocabularyPracticePath, workspacePath, workspacesPath } from './lib/routes'
...
export default function App() {
  ...
  const [route, setRoute] = useState(initialRoute.page === 'quiz' || initialRoute.page === 'workspaces' || initialRoute.page === 'vocabulary' || initialRouteLanguage || initialRouteChat || initialRouteWorkspace ? initialRoute : { page: 'home', chatId: null })
  ...
  useEffect(() => { const timer = setTimeout(() => setStorageError(!saveState({ theme, providers, activeProvider, models, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages, workspaces, conversations, activeId, activeWorkspaceId, temperature, wideChat, sidebarWidth, sidebarVisible, pronunciationLanguage, pronunciationVoice })), 300); return () => clearTimeout(timer) }, [theme, providers, activeProvider, models, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages, workspaces, conversations, activeId, activeWorkspaceId, temperature, wideChat, sidebarWidth, sidebarVisible, pronunciationLanguage, pronunciationVoice])
  useEffect(() => {
    let frame
    let timer
  ...
  }, [initialRoute.page, initialRouteChat, initialRouteWorkspace, initialRouteLanguage])
  useEffect(() => {
    const onPopState = () => {
      ...
      } else if (nextRoute.page === 'workspaces' || nextRoute.page === 'vocabulary') {
        setRoute(nextRoute)
  ...
  }, [conversations, workspaces, vocabularyLanguages])
  useEffect(() => { const vocabularyLanguage = vocabularyLanguages.find((language) => language.id === route.languageId); document.title = route.page === 'chat' ? `${activeConversation?.title || 'Chat'} · ShinkuChat` : route.page === 'workspace' ? `${activeWorkspace?.name || 'Workspace'} · ShinkuChat` : route.page === 'quiz' ? 'Quiz · ShinkuChat' : route.page === 'vocabulary-practice' ? `${vocabularyLanguage?.name || 'Vocabulary'} Practice · ShinkuChat` : route.page === 'vocabulary-language' ? `${vocabularyLanguage?.name || 'Vocabulary'} · ShinkuChat` : route.page === 'vocabulary' ? 'My Vocabulary · ShinkuChat' : 'ShinkuChat · Home' }, [route.page, route.languageId, activeConversation?.title, activeWorkspace?.name, vocabularyLanguages])
  ...
  const retryChat = async (assistantId) => {
    ...
    const history = messages.slice(0, assistantIndex).filter((item, index) => !(index === 0 && item.role === 'assistant' && (item.content.startsWith('Halo! Saya **Chatty**') || item.content.startsWith('Halo. Hubungkan provider'))))
  ...
  const completeJson = async (messages, maxTokens = OUTPUT_RESERVE) => {
    if (!provider.baseUrl || !provider.model) throw new Error('Connect a provider and select a model first.')
    let output = ''
    await streamCompletion({ provider: normalizeProvider(provider), model: provider.model, messages, settings: { temperature: 0.2, maxTokens }, onToken: (token) => { output += token } })
  ...
  return <div className="relative flex h-dvh overflow-hidden bg-white font-sans text-sm text-slate-800 [&_*]:!font-sans [&_a]:cursor-pointer [&_button:not(:disabled)]:cursor-pointer [&_button:disabled]:cursor-not-allowed [&_code]:!font-mono [&_input:not(:disabled)]:cursor-text [&_input:disabled]:cursor-not-allowed [&_input[type=button]:not(:disabled)]:cursor-pointer [&_input[type=checkbox]:not(:disabled)]:cursor-pointer [&_input[type=color]:not(:disabled)]:cursor-pointer [&_input[type=file]:not(:disabled)]:cursor-pointer [&_input[type=radio]:not(:disabled)]:cursor-pointer [&_input[type=range]:not(:disabled)]:cursor-pointer [&_input[type=reset]:not(:disabled)]:cursor-pointer [&_input[type=submit]:not(:disabled)]:cursor-pointer [&_label]:cursor-pointer [&_option]:cursor-pointer [&_pre]:!font-mono [&_pre_*]:!font-mono [&_select:not(:disabled)]:cursor-pointer [&_select:disabled]:cursor-not-allowed [&_summary]:cursor-pointer [&_textarea:not(:disabled)]:cursor-text [&_textarea:disabled]:cursor-not-allowed [&_[contenteditable=true]]:cursor-text [&_[draggable=true]]:cursor-grab [&_[role=button]]:cursor-pointer [&_[role=checkbox]]:cursor-pointer [&_[role=combobox]]:cursor-pointer [&_[role=link]]:cursor-pointer [&_[role=listbox]]:cursor-default [&_[role=menuitem]]:cursor-pointer [&_[role=option]]:cursor-pointer [&_[role=radio]]:cursor-pointer [&_[role=slider]]:cursor-pointer [&_[role=switch]]:cursor-pointer [&_[role=tab]]:cursor-pointer transition-[background-color,color] duration-500 dark:bg-slate-950 dark:text-slate-100">
    ...
    <aside className={`fixed inset-y-0 left-0 z-30 flex max-w-[calc(100vw-24px)] ${sidebarWidths[sidebarWidth]} flex-col overflow-hidden border-r border-purple-100/70 bg-white/80 p-3 shadow-[16px_0_40px_rgba(76,29,149,0.08)] backdrop-blur-xl transition-transform duration-500 ease-out dark:border-white/5 dark:bg-slate-950/80 dark:shadow-[16px_0_45px_rgba(126,34,206,0.08)] ${sidebarVisible ? 'md:static md:translate-x-0' : 'md:fixed md:-translate-x-full'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="relative space-y-1 border-t border-purple-100 pt-2 dark:border-white/5"><button onClick={() => downloadJson({ workspaces, conversations, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages }, 'shinkuchat-export.json')} className={`${button} w-full justify-start gap-2 px-2`}><DownloadSimple size={17} />Export data</button><button onClick={() => setSettingsOpen(true)} className={`${button} w-full justify-start gap-2 px-2`}><GearSix size={17} />Provider & model</button></div>
    </aside>
    ...
    <main className={`relative z-10 flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden transition-colors duration-500 ${workspaceBackgrounds[activeWorkspace?.color] || workspaceBackgrounds.white}`}>
      ...
      <button type="button" onClick={() => { if (window.matchMedia('(min-width: 768px)').matches) setSidebarVisible((value) => !value); else setSidebarOpen((value) => !value) }} aria-label={sidebarOpen || sidebarVisible ? 'Hide sidebar' : 'Show sidebar'} title={sidebarOpen || sidebarVisible ? 'Hide sidebar' : 'Show sidebar'} className={`fixed top-1/2 z-40 flex h-16 w-9 -translate-y-1/2 items-center justify-center rounded-r-xl bg-gradient-to-b from-purple-600 to-purple-800 text-white shadow-[0_10px_28px_rgba(126,34,206,0.34)] transition-[left,background-color,color,transform,box-shadow] duration-500 ease-out hover:scale-105 hover:shadow-[0_14px_32px_rgba(126,34,206,0.42)] focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/10 active:scale-95 motion-reduce:transform-none dark:from-purple-400 dark:to-purple-600 dark:text-slate-950 dark:shadow-[0_10px_28px_rgba(168,85,247,0.28)] ${sidebarOpen ? 'left-[calc(100vw-24px)]' : 'left-0'} ${sidebarVisible ? sidebarTogglePositions[sidebarWidth] : 'md:left-0'}`}>{sidebarOpen || sidebarVisible ? <CaretLeft size={18} weight="bold" /> : <CaretRight size={18} weight="bold" />}</button>

      {route.page === 'vocabulary' || route.page === 'vocabulary-language' || route.page === 'vocabulary-practice' ? <VocabularyPage route={route} languages={vocabularyLanguages} onAddLanguage={addVocabularyLanguage} onDeleteLanguage={deleteVocabularyLanguage} onOpenLanguage={openVocabularyLanguage} onOpenPractice={openVocabularyPractice} onBack={() => navigateVocabulary(true)} onAddWord={addVocabularyWord} onDeleteWord={deleteVocabularyWord} onUpdateUserLanguage={updateVocabularyUserLanguage} onUpdatePronunciation={updateVocabularyPronunciation} providerReady={Boolean(provider.baseUrl && provider.model)} onExplainWord={explainVocabularyWord} onGeneratePractice={generateVocabularyPractice} /> : route.page === 'quiz' ? <QuizPage route={route} onNavigate={navigateQuizRoute} providerReady={Boolean(provider.baseUrl && provider.model)} categories={quizCategories} quizzes={savedQuizzes} attempts={quizAttempts} onAddCategory={(category) => setQuizCategories((all) => [...all, category])} onDeleteCategory={deleteQuizCategory} onSaveQuiz={(quiz) => setSavedQuizzes((all) => [quiz, ...all])} onSaveAttempt={(attempt) => setQuizAttempts((all) => [attempt, ...all])} onOpenSettings={() => setSettingsOpen(true)} onGenerate={generateQuiz} onGrade={gradeQuiz} /> : route.page === 'home' || route.page === 'workspaces' || route.page === 'workspace' ? <HomePortal key={route.page === 'workspace' ? route.workspaceId : route.page} workspaces={workspaces} conversations={conversations} onOpenChat={selectChat} onNewChat={newChat} onAddWorkspace={() => setWorkspaceDialog({ open: true, workspace: null })} onDeleteWorkspace={deleteWorkspace} onOpenQuiz={navigateQuiz} onOpenVocabulary={navigateVocabulary} initialView={route.page === 'workspaces' ? 'chat' : 'root'} initialWorkspaceId={route.page === 'workspace' ? route.workspaceId : null} onLeaveWorkspace={navigateWorkspaces} /> : <>
    ...
    <SelectionToolbar selection={selectionAction} chatActions={selectionAction?.chatActions} languageCode={pronunciationLanguage} voiceURI={pronunciationVoice} onLanguageChange={(language) => { setPronunciationLanguage(language); setPronunciationVoice('') }} onVoiceChange={setPronunciationVoice} onPronounce={(text) => speakWord({ text, languageCode: pronunciationLanguage, voiceURI: pronunciationVoice })} onFollowUp={(text) => applySelection(text, false)} onAsk={(text) => applySelection(text, true)} onClose={() => setSelectionAction(null)} />
