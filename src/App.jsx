...
export default function App() {
  ...
  const [mindMaps, setMindMaps] = useState(stored?.mindMaps || [])
  ...
  const saveMindMap = (map) => setMindMaps((all) => [map, ...all.filter((item) => item.id !== map.id)])
  const deleteMindMap = (mapId) => confirmAction('Delete this mind map?', 'All nodes and their branches will be permanently removed.', () => setMindMaps((all) => all.filter((item) => item.id !== mapId)))
  const generateMindMap = async ({ topic, instructions }) => {
    const result = normalizeMindMap(await completeJson([
      { role: 'system', content: 'Return only compact valid JSON: {"title":"string","nodes":[{"id":"unique-short-id","label":"concise label","parentId":"parent id or null"}]}. Build a useful hierarchical mind map with one root, 4-7 major branches, and 2-4 concise child nodes per major branch. Maximum 35 nodes and 4 levels. Every parentId must reference an earlier node. No Markdown or extra text.' },
      { role: 'user', content: `Topic: ${topic}\nAdditional direction: ${instructions || 'Map the topic comprehensively and practically.'}` },
    ], 3000), topic)
    const map = { ...result, id: id(), createdAt: Date.now(), updatedAt: Date.now() }
    saveMindMap(map)
    return map
  }
