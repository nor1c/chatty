...
import { ArrowsInLineHorizontal, ArrowsOutLineHorizontal, ArrowDown, ArrowRight, ArrowSquareOut, BookOpen, ChatCircle, DownloadSimple, Exam, GearSix, House, List, PaperPlaneRight, Plus, Sparkle, Stop, Trash, X } from '@phosphor-icons/react'
...
import { chatPath, quizPaths, readRoute, vocabularyLanguagePath, vocabularyPath, workspacePath, workspacesPath } from './lib/routes'
...
export default function App() {
  ...
  useEffect(() => { const timer = setTimeout(() => setStorageError(!saveState({ theme, providers, activeProvider, models, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages, workspaces, conversations, activeId, activeWorkspaceId, temperature, wideChat, sidebarWidth, sidebarVisible })), 300); return () => clearTimeout(timer) }, [theme, providers, activeProvider, models, quizCategories, savedQuizzes, quizAttempts, vocabularyLanguages, workspaces, conversations, activeId, activeWorkspaceId, temperature, wideChat, sidebarWidth, sidebarVisible])
  ...
  useEffect(() => {
    if ((initialRoute.page === 'chat' && !initialRouteChat) || (initialRoute.page === 'workspace' && !initialRouteWorkspace) || (initialRoute.page === 'vocabulary-language' && !initialRouteLanguage)) window.history.replaceState({}, '', '/')
  ...
  useEffect(() => {
    const onPopState = () => {
      ...
      if (nextRoute.page === 'chat' && nextChat) {
        ...
        setRoute(nextRoute)
      } else if (nextRoute.page === 'vocabulary-language' && vocabularyLanguages.some((language) => language.id === nextRoute.languageId)) {
        setRoute(nextRoute)
      } else {
  ...
  useEffect(() => { const vocabularyLanguage = vocabularyLanguages.find((language) => language.id === route.languageId); document.title = route.page === 'chat' ? `${activeConversation?.title || 'Chat'} · ShinkuChat` : route.page === 'workspace' ? `${activeWorkspace?.name || 'Workspace'} · ShinkuChat` : route.page === 'quiz' ? 'Quiz · ShinkuChat' : route.page === 'vocabulary-language' ? `${vocabularyLanguage?.name || 'Vocabulary'} · ShinkuChat` : route.page === 'vocabulary' ? 'My Vocabulary · ShinkuChat' : 'ShinkuChat · Home' }, [route.page, route.languageId, activeConversation?.title, activeWorkspace?.name, vocabularyLanguages])
  ...
  const explainVocabularyWord = async (language, word) => {
    ...
    const details = await completeJson([
      { role: 'system', content: `Return only valid JSON with this shape: {"translation":"string","meaning":"string","explanation":"string","example":"string"}. Explain the ${language.name} word entirely in ${userLanguage}. Every field is required and must be non-empty. Keep each field concise. The example must be in ${language.name} and contain the exact target word.` },
      { role: 'user', content: `Target word: ${word.word}` },
    ], 1200)
    const fields = ['translation', 'meaning', 'explanation', 'example']
  ...
  return <div className="relative flex h-dvh overflow-hidden bg-white font-sans text-sm text-slate-800 [&_*]:!font-sans [&_code]:!font-mono [&_pre]:!font-mono [&_pre_*]:!font-mono transition-[background-color,color] duration-500 dark:bg-slate-950 dark:text-slate-100">
    ...
    <aside className={`fixed inset-y-0 left-0 z-30 flex max-w-[calc(100vw-24px)] ${sidebarWidths[sidebarWidth]} flex-col overflow-hidden border-r border-purple-100/70 bg-white/80 p-3 shadow-[16px_0_40px_rgba(76,29,149,0.08)] backdrop-blur-xl transition-transform duration-500 ease-out dark:border-white/5 dark:bg-slate-950/80 dark:shadow-[16px_0_45px_rgba(126,34,206,0.08)] ${sidebarVisible ? 'md:static md:translate-x-0' : 'md:fixed md:-translate-x-full'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="mt-3 space-y-1"><button onClick={navigateHome} aria-current={route.page === 'home' ? 'page' : undefined} className={`${button} w-full justify-start gap-2 px-2 ${route.page === 'home' ? 'bg-purple-100 text-purple-800 dark:bg-purple-500/15 dark:text-purple-200' : ''}`}><House size={17} weight={route.page === 'home' ? 'fill' : 'regular'} />Home</button><button onClick={navigateWorkspaces} aria-current={route.page === 'workspaces' || route.page === 'workspace' || route.page === 'chat' ? 'page' : undefined} className={`${button} w-full justify-start gap-2 px-2 ${route.page === 'workspaces' || route.page === 'workspace' || route.page === 'chat' ? 'bg-purple-100 text-purple-800 dark:bg-purple-500/15 dark:text-purple-200' : ''}`}><ChatCircle size={17} weight={route.page === 'workspaces' || route.page === 'workspace' || route.page === 'chat' ? 'fill' : 'regular'} />Chat</button><button onClick={navigateVocabulary} aria-current={route.page === 'vocabulary' || route.page === 'vocabulary-language' ? 'page' : undefined} className={`${button} w-full justify-start gap-2 px-2 ${route.page === 'vocabulary' || route.page === 'vocabulary-language' ? 'bg-purple-100 text-purple-800 dark:bg-purple-500/15 dark:text-purple-200' : ''}`}><BookOpen size={17} weight={route.page === 'vocabulary' || route.page === 'vocabulary-language' ? 'fill' : 'regular'} />My Vocabulary</button><button onClick={navigateQuiz} aria-current={route.page === 'quiz' ? 'page' : undefined} className={`${button} w-full justify-start gap-2 px-2 ${route.page === 'quiz' ? 'bg-purple-100 text-purple-800 dark:bg-purple-500/15 dark:text-purple-200' : ''}`}><Exam size={17} weight={route.page === 'quiz' ? 'fill' : 'regular'} />Quiz</button></div>
    ...
    <main className={`relative z-10 flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden transition-colors duration-500 ${workspaceBackgrounds[activeWorkspace?.color] || workspaceBackgrounds.white}`}>
      ...
      {route.page === 'vocabulary' || route.page === 'vocabulary-language' ? <VocabularyPage route={route} languages={vocabularyLanguages} onAddLanguage={addVocabularyLanguage} onOpenLanguage={openVocabularyLanguage} onBack={() => navigateVocabulary(true)} onAddWord={addVocabularyWord} onUpdateUserLanguage={updateVocabularyUserLanguage} providerReady={Boolean(provider.baseUrl && provider.model)} onExplainWord={explainVocabularyWord} /> : route.page === 'quiz' ? <QuizPage route={route} onNavigate={navigateQuizRoute} providerReady={Boolean(provider.baseUrl && provider.model)} categories={quizCategories} quizzes={savedQuizzes} attempts={quizAttempts} onAddCategory={(category) => setQuizCategories((all) => [...all, category])} onSaveQuiz={(quiz) => setSavedQuizzes((all) => [quiz, ...all])} onSaveAttempt={(attempt) => setQuizAttempts((all) => [attempt, ...all])} onOpenSettings={() => setSettingsOpen(true)} onGenerate={generateQuiz} onGrade={gradeQuiz} /> : route.page === 'home' || route.page === 'workspaces' || route.page === 'workspace' ? <HomePortal key={route.page === 'workspace' ? route.workspaceId : route.page} workspaces={workspaces} conversations={conversations} onOpenChat={selectChat} onNewChat={newChat} onAddWorkspace={() => setWorkspaceDialog({ open: true, workspace: null })} onOpenQuiz={navigateQuiz} onOpenVocabulary={navigateVocabulary} initialView={route.page === 'workspaces' ? 'chat' : 'root'} initialWorkspaceId={route.page === 'workspace' ? route.workspaceId : null} onLeaveWorkspace={navigateWorkspaces} /> : <>
