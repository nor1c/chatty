...
import { ArrowLeft, ArrowRight, BookOpen, CaretDown, Check, GameController, GearSix, ListBullets, MagnifyingGlass, Plus, Shuffle, SpeakerHigh, SpinnerGap, SquaresFour, Trash, Translate, X } from '@phosphor-icons/react'
...
function Workspace({ language, onBack, onDeleteLanguage, onOpenPractice, onAddWord, onDeleteWord, onUpdateUserLanguage, onUpdatePronunciation, providerReady, onExplainWord }) {
  ...
  return <Page>
    ...
    <div className={`${surface} mt-2 ${view === 'list' ? 'divide-y divide-slate-100 dark:divide-slate-800' : 'flex flex-wrap gap-2'}`}>{words.map((item) => <div key={item.id} className={`${view === 'list' ? 'flex w-full items-center gap-2 py-1' : 'flex w-auto items-center gap-1 rounded-lg bg-purple-50 p-1 dark:bg-purple-500/10'}`}><button type="button" onClick={() => openDetails(item)} className={`${view === 'list' ? 'min-w-0 flex-1 px-2 py-2 text-left' : 'max-w-56 px-2 py-1.5 text-left'} rounded-lg transition-colors duration-300 ease-out hover:bg-purple-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/10 dark:hover:bg-purple-500/10`}><strong className="block truncate">{item.word}</strong><span className="mt-0.5 block truncate text-xs font-normal text-slate-500 dark:text-slate-400">{readablePronunciation(item.pronunciation) || 'Buka untuk melengkapi'}</span></button><span className="ml-auto flex shrink-0 items-center gap-0"><button type="button" onClick={() => pronunciation.speak(item.word)} disabled={!pronunciation.supported} aria-label={`Pronounce ${item.word}`} title={`Pronounce ${item.word}`} className="flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-md text-purple-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/10 disabled:cursor-not-allowed disabled:opacity-40 dark:text-purple-300"><SpeakerHigh size={16} /></button><button type="button" onClick={() => { pronunciation.stop(); setOpenWord(null); onDeleteWord(language.id, item.id) }} aria-label={`Delete ${item.word}`} title={`Delete ${item.word}`} className="flex h-8 w-8 shrink-0 cursor-pointer items-center justify-center rounded-md text-slate-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/10 dark:text-slate-400"><Trash size={16} /></button></span></div>)}{!words.length && <p className="w-full py-4 text-center text-sm text-slate-500">Add your first word to begin.</p>}</div>
