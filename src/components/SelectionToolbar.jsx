...
function VoicePicker({ language, voiceURI, rate, onChange, onRateChange, onClose }) {
  ...
  return <div role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }} className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/50 px-3 py-3 backdrop-blur-sm">
    <section role="dialog" aria-modal="true" aria-labelledby="voice-picker-title" className="flex max-h-[min(36rem,calc(100dvh-24px))] w-full max-w-md flex-col overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-[0_18px_50px_rgba(15,23,42,0.24)] dark:border-slate-700 dark:bg-slate-900 dark:shadow-[0_18px_50px_rgba(0,0,0,0.45)]">
      ...
      <fieldset className="mt-3"><legend className="text-[13px] font-medium text-slate-700 dark:text-slate-200">Pronunciation speed</legend><div className="mt-1 grid grid-cols-3 gap-2">{[[0.5, 'Slow'], [1, 'Normal'], [2, 'Fast']].map(([value, label]) => <button type="button" key={value} aria-pressed={rate === value} onClick={() => onRateChange(value)} className={`h-9 rounded-lg text-sm font-medium transition-colors duration-300 ease-out focus:outline-none focus:ring-2 focus:ring-purple-500/10 ${rate === value ? 'bg-purple-700 text-white' : 'bg-slate-100 text-slate-600 hover:bg-purple-100 hover:text-purple-800 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-purple-500/15'}`}>{label}<span className="ml-1 text-xs opacity-70">{value}×</span></button>)}</div></fieldset>
...
export default function SelectionToolbar({ selection, chatActions = false, askActions = false, languageCode, voiceURI, pronunciationRate, onLanguageChange, onVoiceChange, onPronunciationRateChange, onPronounce, onFollowUp, onAsk, onClose }) {
  ...
  return <div ref={toolbarRef} data-selection-toolbar role="toolbar" aria-label="Actions for selected text" className="fixed z-[65] flex max-w-[calc(100vw-16px)] -translate-x-1/2 flex-wrap items-center justify-center gap-1 rounded-lg border border-slate-200 bg-white p-1 shadow-[0_14px_35px_rgba(15,23,42,0.18)] dark:border-slate-700 dark:bg-slate-900">
    ...
    {voicePickerOpen && createPortal(<VoicePicker language={language} voiceURI={voiceURI} rate={pronunciationRate} onChange={onVoiceChange} onRateChange={onPronunciationRateChange} onClose={() => setVoicePickerOpen(false)} />, document.body)}
  </div>
}
